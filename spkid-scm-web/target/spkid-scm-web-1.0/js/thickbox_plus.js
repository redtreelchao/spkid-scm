...
$("a.thickbox").click(function(){
  var t = this.title;
  TB_show_view(t,this.href);
  this.blur();
  return false;
});
}

function TB_LAS_launch() {
...
function TB_show_view(caption, url) {
	TB_show(caption, url, function(){});
}

function TB_show(caption, url,callback) { //function called when the user clicks on a thickbox link
	try {
		$("body")
		...
		if(urlType == '.htm' || urlType == '.html'){//code to show html pages
			...
			$("#TB_window").append("<div id='TB_closeAjaxWindow'><span class='TB_titleWindow'>"+caption+"</span><a href='#' id='TB_closeWindowButton'>关闭</a></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px;'></div>");
			if(callback != null){
				$("#TB_closeWindowButton").bind("click",function(){
					TB_remove_Call(callback)
				});
			}else
			    $("#TB_closeWindowButton").click(TB_remove);
...
function TB_remove_Call(callback){
	callback();
	TB_remove();
}
...
function TB_remove() {
	...
	if ($('#TickBoxReload').length>0){
	    $("#TB_window").fadeOut(0,function(){$('#TB_window,#TB_overlay,#TB_load').remove();});
	    location.reload();
	} else {
	$("#TB_window").fadeOut(0,function(){$('#TB_window,#TB_overlay,#TB_load').remove();}); 
	}
...
function TB_position() {
	...
	left: ((w - TB_WIDTH)/2)+"px", top: ($(window).scrollTop() + (h - TB_HEIGHT)/2)+"px" });
